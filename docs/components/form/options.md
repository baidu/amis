...
**选择器表单项** 是指那些（例如下拉选择框）具有[选择器特性](#%E9%80%89%E6%8B%A9%E5%99%A8%E7%89%B9%E6%80%A7)的表单项
...
`options`属性配置的对象数组就是`select`组件的选项组。
...
有时候我们想默认选中一个选项，但是`options`又是远程拉取的，无法确定默认值是啥，这时候，需要在`source`接口中返回`value`，来动态设置默认值，接口返回数据结构如下：
