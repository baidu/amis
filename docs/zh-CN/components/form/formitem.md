...
设置 autoFillApi.showSuggestion 为 true；同时在 autoFillApi 中配置如下示例参数，可以进行数据的参照录入「当前表单项聚焦或者值变化时弹出 dialog/drawer/popOver 供用户操作」例如如下配置
  ...
  "body": [
    {
      ...
      "name": "browser",
      "autoFillApi": {
        "url": "/api/mock2/form/autoFillApi",
        "showSuggestion": true,
        "multiple": true,
        "fillMapping": { "__target": "combo", "name": "${name}", "version": "${version}" },
...
| autoFillApi                | [表达式](../../../docs/types/api)                  |           | 自动填充配置                                                                                        |
| autoFillApi.silent         | `boolean`                                          |           | 是否展示数据格式错误提示，默认为 true                                                               |
| autoFillApi.showSuggestion | `boolean`                                          |           | 是否使用参照录入                                                                                    |
| autoFillApi.fillMappinng   | [SchemaNode](../../docs/types/schemanode)          |           | showSuggestion 为 true 时，参照录入数据映射配置                                                     |
| autoFillApi.trigger        | `string`                                           |           | showSuggestion 为 true 时，参照录入支持的触发方式，目前支持 change「值变化」｜ focus 「表单项聚焦」 |
| autoFillApi.mode           | `string`                                           |           | showSuggestion 为 true 时，参照弹出方式 dialog, drawer, popOver                                     |
| autoFillApi.position       | `string`                                           |           | showSuggestion 为 true 时，参照录入 mode 为 popOver 时，可配置弹出位置                              |
| autoFillApi.size           | `string`                                           |           | showSuggestion 为 true 时，参照录入 mode 为 dialog 时，可设置大小                                   |
| autoFillApi.multiple       | `boolean`                                          | false     | showSuggestion 为 true 时，参照录入回填单条 ｜ 多条                                                 |
| autoFillApi.columns        | `Array<Column>`                                    |           | showSuggestion 为 true 时，数据展示列配置                                                           |
| autoFillApi.filter         | [SchemaNode](../../docs/types/schemanode)          |           | showSuggestion 为 true 时，数据查询过滤条件                                                         |
