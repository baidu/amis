...
export interface ListenerAction {
  actionType: 'rendererEvent' | LogicActionType | 'custom' | string; // 动作类型 逻辑动作|自定义（脚本支撑）|reload|url|ajax|dialog|drawer 其他扩充的组件动作
  eventName?: string; // 事件名称，actionType: rendererEvent
  description?: string; // 事件描述，actionType: rendererEvent
...
export interface ListenerContext {
  logicData?: any; // 逻辑处理产生的数据
  loopStatus?: LoopStatus; // 循环执行状态
  [propName: string]: any;
}
...
export const runActionTree = async (
  ...
  for (const action of handlers) {
    ...
    // 这些节点的子节点运行逻辑由节点内部实现
    await runAction(actionInstrance, action, context, rendererEvent);

    // 阻止原有动作执行
    action.preventDefault && rendererEvent.preventDefault();
    // 阻止后续动作执行
    action.stopPropagation && rendererEvent.stopPropagation();
...
export const runAction = async (
  ...
  await actionInstrance.run(actionConfig, context, rendererEvent);
};
