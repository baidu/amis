...
const defaultOptions: RenderOptions = {
  ...
  // 返回解绑函数
  bindEvent(context: any) {
    const listeners: EventListeners = context.props.$schema.onEvent;
    if (listeners) {
      // 暂存
  ...
  async dispatchEvent(
    ...
    if (!broadcast) {
      const eventConfig = context.props.onEvent?.[eventName];
    ...
    // 广播动作，把事件参数补上
    if (broadcast) {
      rendererEvent.context.eventData = data;
    }

    // 过滤&排序
    const listeners = this.rendererEventListeners
      .filter((item: RendererEventListener) => {
        // 直接对比context有问题，这里context可能是ActionRenderer，而上面存的是具体的组件渲染器。后面有了id，可以直接用id
        return (
          item.type === eventName &&
          (broadcast ? true : item.context.props.$path === context.props.$path)
        );
      })
      .sort(
    ...
    for (let listener of listeners) {
      await runActionTree(listener.actions, listener.context, rendererEvent);
