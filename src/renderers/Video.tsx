...
export default class Video extends React.Component<VideoProps, VideoState> {
    ...
    playerRef(player: any) {
        ...
        player.subscribeToStateChange((state: any) => {
            ...
            if (!state.paused) {
                if (currentPlaying && currentPlaying.video && currentPlaying !== player) {
    ...
    renderFrames() {
        ...
        return (
            <div className={cx(`pos-rlt ${ns}Video-frameList`, framesClassName)} ref={this.frameRef}>
                ...
                {jumpFrame ? <span ref={this.cursorRef} className={`${ns}Video-cursor`} /> : null}
            </div>
        );
    }
