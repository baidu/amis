...
export interface TimelineItemSchema extends Omit<BaseSchema, 'type'> {
   ...
   * detail折叠时文案
   */
  detailCollapseText?: string;

  /**
   * detail展开时文案
   */
  detailExpandText?: string;
...
export function TimelineCmpt(props: TimelineProps) {
  ...
  // 获取源数据
  const timelineItemsRow = (resolveVariable(source, data) as Array<TimelineItemSchema>)
                      || config
                      || items
                      || [];
  // 渲染内容
  ...
  // 处理源数据
  const resolveTimelineItems = timelineItemsRow.map((timelineItem, i) => {
    return {
      ...timelineItem,
