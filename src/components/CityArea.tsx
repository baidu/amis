...
export interface AreaProps extends LocaleProps,
    ThemeProps {
  value: any;
  /**
   * 允许选择城市？
...
const CityArea = memo<AreaProps>((props) => {
  ...
  const [confirmValues, setConfirmValues] = useState<Array<PickerObjectOption>>();
  ...
  const onChange = (columnValues: Array<AreaColumnOption>, columnIndex: number) => {
  ...
  const propsChange = () => {
    ...
    const code = allowDistrict && district ? district : allowCity && city ? city : province;
  ...
  const updateColumns = () => {
    ...
    const cityColumn = city ? db.city[provience].map((code: number) => {
      return { text: db[code], value: code, disabled};
    }) : [];
    const districtColumn = city && district ? db.district[provience][city].map((code: number) => {
      return { text: db[code], value: code, disabled };
    }) : [];
    const columns = [
  ...
  const result = confirmValues?.filter(item => item?.value)?.map(item => item.text).join(delimiter);
    ...
    <div className={cx(`CityArea`)}>
      ...
      {allowStreet && values[0] ? (
          <input
            ...
            value={street}
            onChange={(e: React.ChangeEvent<HTMLInputElement>)=> setStreet(e.currentTarget.value)}
