...
export class Elevator extends React.Component<ElevatorProps, ElevatorState> {
    static defaultProps: Pick<ElevatorProps, 'navClassName' | 'floorClassName'> = {
        navClassName: '',
        floorClassName: ''
    };
    ...
    componentDidMount() {
        ...
        // add scroll event
        const floorRootDom = this.contentDom &&
            this.contentDom.current as HTMLElement;
        ...
        // 收集楼层offsetTop
        children &&
            React.Children.forEach(children, (floor: FloorComponent, index: number) => {
                offsetArr.push({
    ...
    handleSelect(key: string | number) {
        ...
        // 如果已经滚到底就不去更新导航选中了
        if (floorRootDom.scrollHeight - floorRootDom.scrollTop < floorRootDom.clientHeight) {
            // fire event
            this.fireSelect(key);
        }
    ...
    renderFloor(floor: any, index: number) {
        ...
        const active =
            activeProp === undefined && index === 0 ? name : activeProp;
    ...
    render() {
        ...
        return (
            ...
            >
                <ul className={cx('Elevator-nav-wrap', navClassName)} role="elevatorlist">
                    {children.map((nav, index) => this.renderNav(nav, index))}
                </ul>

                <div className={cx('Elevator-floor-wrap', floorClassName)}
                    ref={this.contentDom}
                >
