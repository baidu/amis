...
export const filterDate = (value: string, data: object = {}, format = 'X'): moment.Moment => {
    let m;
...
export const resolveVariableAndFilter = (path: string, data: object = {}, defaultFilter: string = '| html'): any => {
    if (!path) {
        return undefined;
    }
    ...
    Object.keys(to).forEach(key => {
        ...
        if (key === '&' && value === '$$') {
            ...
            (ret as PlainObject)[key] = value.map((value: any) =>
                isPlainObject(value) ? dataMapping(value, from) : resolveMapping(value, from)
            );
