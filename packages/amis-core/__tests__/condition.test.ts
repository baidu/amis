import {guid, resolveConditions} from '../src/utils/';
...
test(`condition`, async () => {
  ...
  expect(await resolveConditions(conditions1, data)).toBe(
    'true && false && true && false'
  );
  expect(await resolveConditions(conditions2, data)).toBe(
    'false || false || true || true'
  );
  expect(await resolveConditions(conditions3, data)).toBe(
    'true && true && false && true'
  );
  expect(await resolveConditions(conditions4, data)).toBe(
    'false && true && true'
  );
  expect(await resolveConditions(conditions5, data)).toBe(
    'false && false && true && false'
  );
  expect(await resolveConditions(conditions6, data)).toBe('true && false');
  expect(await resolveConditions(conditions7, data)).toBe(
    'true && true && false'
  );
  expect(await resolveConditions(conditions8, data)).toBe(
