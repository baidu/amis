...
import { setSchemaTpl, getSchemaTpl, defaultValue, isObject } from 'amis-editor-core';
...
setSchemaTpl('labelHide', function () {
    return getSchemaTpl('switch', {
        ...
        visibleOn: '__props__.formMode === "horizontal" || data.mode === "horizontal" || data.label === false'
    });
});
...
setSchemaTpl('tabs', function (config) {
    return {
        ...
        tabs: config
            ...
            .map(function (item) {
            var _a;
            var newSchema = __assign(__assign({}, item), { body: Array.isArray(item.body) ? flatten(item.body) : [item.body] });
            // 新版配置面板空隙在子组件中，兼容一下
...
setSchemaTpl('collapse', function (config) {
    return {
        ...
        body: config
            .filter(function (item) { return item; })
            .map(function (item) { return (__assign(__assign({ type: 'collapse', headingClassName: 'editor-prop-config-head', bodyClassName: 'editor-prop-config-body' }, item), { body: flatten(item.body) })); })
    };
});
setSchemaTpl('fieldSet', function (config) {
    return __assign(__assign({ collapsable: true, collapsed: false }, config), { type: 'fieldset', title: config.title, body: flatten(config.body.filter(function (item) { return item; })) });
});
setSchemaTpl('collapseGroup', function (config, rendererSchema) {
    ...
    var collapseGroupBody = config
        ...
        .map(function (item, index) { return (__assign(__assign({ type: 'collapse', headingClassName: 'ae-formItemControl-header', bodyClassName: 'ae-formItemControl-body' }, item), { key: "".concat(currentKey, "_").concat(item.key || index.toString()), body: flatten(item.body) })); });
    return {
        type: 'collapse-group',
        key: currentKey,
...
setSchemaTpl('valueFormula', function (config) {
    ...
    if ((config === null || config === void 0 ? void 0 : config.useSelectMode) &&
        curRendererSchema &&
        curRendererSchema.options) {
        curRendererSchema = __assign(__assign({}, curRendererSchema), { type: 'select' });
    }
            ...
            body: [
                {
                    ...
                    label: (config === null || config === void 0 ? void 0 : config.label) || '默认值',
...
setSchemaTpl('className', function (schema) {
    return __assign(__assign({ type: 'ae-classname', name: 'className' }, (schema || {})), { label: tipedLabel((schema === null || schema === void 0 ? void 0 : schema.label) || 'CSS 类名', '有哪些辅助类 CSS 类名？请前往 <a href="https://baidu.github.io/amis/docs/concepts/style" target="_blank">样式说明</a>，除此之外你可以添加自定义类名，然后在系统配置中添加自定义样式。') });
});
/**
 * combo 组件样式包装调整
 */
setSchemaTpl('combo-container', function (config) {
    var _a;
    if (isObject(config)) {
        var itemsWrapperClassName = config.type === 'combo' &&
            'ae-Combo-items ' + ((_a = config.itemsWrapperClassName) !== null && _a !== void 0 ? _a : '');
        return __assign(__assign({}, config), (itemsWrapperClassName ? { itemsWrapperClassName: itemsWrapperClassName } : {}));
    }
    return config;
});
/**
...
setSchemaTpl('status', function (config) {
    return {
        ...
        body: [
            getSchemaTpl('hidden'),
            (config === null || config === void 0 ? void 0 : config.readonly) ? getSchemaTpl('readonly') : null,
            (config === null || config === void 0 ? void 0 : config.disabled) || (config === null || config === void 0 ? void 0 : config.isFormItem)
                ? getSchemaTpl('disabled')
                : null,
            (config === null || config === void 0 ? void 0 : config.isFormItem) ? getSchemaTpl('clearValueOnHidden') : null
        ]
    };
});
setSchemaTpl('autoFill', {
    type: 'input-kv',
    name: 'autoFill',
    label: tipedLabel('自动填充', '将当前已选中的选项的某个字段的值，自动填充到表单中某个表单项中，支持数据映射')
});
...
setSchemaTpl('searchable', function () {
    return getSchemaTpl('switch', {
        label: '可检索',
        name: 'searchable'
    });
...
setSchemaTpl('clearValueOnHidden', function () {
    return getSchemaTpl('switch', {
        ...
        label: tipedLabel('隐藏时删除字段', '当前表单项隐藏时，表单提交数据中会删除该表单项的值'),
        name: 'clearValueOnHidden'
    });
});
setSchemaTpl('utc', {
    type: 'switch',
    label: tipedLabel('UTC转换', '开启后，提交数据和展示数据将进行UTC转换；存在跨地域用户的应用建议开启'),
    name: 'utc',
    inputClassName: 'is-inline'
});
setSchemaTpl('embed', {
...
setSchemaTpl('formulaControl', function (schema) {
    ...
    return __assign({ type: 'ae-formulaControl' }, schema);
});
/**
 * 日期范围快捷键组件
 */
setSchemaTpl('dateShortCutControl', function (schema) {
    if (schema === void 0) { schema = {}; }
    return __assign({ type: 'ae-DateShortCutControl' }, schema);
});
setSchemaTpl('eventControl', function (schema) {
    if (schema === void 0) { schema = {}; }
    return __assign({ type: 'ae-eventControl', mode: 'normal' }, schema);
});
setSchemaTpl('data', {
    type: 'input-kv',
    name: 'data',
