...
const builtInActionPanelData = (actionType: string, actionPanel: any) => {
  const {label, description, panelBody, ...rest} = actionPanel;
  const schema: any = panelBody;
  return {
    actionType,
    actionLabel: label,
    description,
 ...
 * @param actionPanel
 */
export const registerActionPanel = (actionType: string, actionPanel?: any) => {
  const {label, tag, panelBody} = actionPanel;
  if (!actionType) {
