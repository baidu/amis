...
import {COMMON_ACTION_SCHEMA_MAP, FORMITEM_CMPTS} from './config';
...
interface EventControlState {
  ...
  actionConfigItems: ActionConfigItemsMap; // 动作配置信息Map
}
...
> {
  ...
  constructor(props: EventControlProps) {
    ...
    pluginEvents.forEach((event: RendererEvent) => {
  ...
  generateEmptyDefault(events: RendererEvent[]) {
    const onEvent: ActionEventConfig = {};
    events.forEach((event: RendererEvent) => {
  ...
  submitConfig(type: string, config: any) {
    ...
    if (['setValue', 'url', 'link'].includes(action.actionType)) {
      ...
      if (action.args?.api?.messages) {
        const api: any = {};
        Object.entries(action.args.api).forEach(([key, value]) => {
          if (key !== 'messages') {
            api[key] = value;
          }
        });
        action.args = {
          ...action.args,
          api,
          messages: action.args.api.messages
        }
      }
    }
  ...
  formatActionInitConfig(action: ActionConfig) {
    ...
    action.args) {
      if (action.args.messages) {
        action.args.api.messages = action.args.messages;
  ...
  activeActionDialog(
    ...
    eventConfig?.dataSchema?.forEach(ds => {
  ...
  getAcionConfig(action: ActionConfig) {
    ...
    const realActionType = ['component', 'openPage'].includes(actionType)
      ...
      : action.actionType;
    let actionConfig: RendererAction | undefined;
    if (['component', 'openPage'].includes(actionType)) {
      // 尝试从actions中获取desc
      actionConfig = actions[action.__rendererName]?.find(
        (item: RendererAction) => item.actionType === realActionType
      );
    }
    actionConfig = {
      ...(actionConfig || {}),
...
// 获取事件Label文案
export const getEventLabel = (events: RendererEvent[], name: string) =>
  events.find(item => item.eventName === name)?.eventLabel;
// 获取事件描述文案
export const getEventDesc = (events: RendererEvent[], name: string) =>
  events.find(item => item.eventName === name)?.description;
// 获取动作Label文案
export const getActionLabel = (events: RendererAction[], name: string) =>
...
export const getComponentTreeByType = (
  ...
  const hasActionType = (actions?: ActionTypeNode[]) => {
